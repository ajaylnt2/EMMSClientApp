...
var loadDisplayNames = function () {
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#idAir").text('Air');
            $("#idGas").text('Gas');
            $("#idElectricity").text('Electricity');
            $("#idSteam").text('Steam');
            if (data.length !== 0) {
                for (var len = 0; len < data.length; len++) {
                    if (data[len].WageType === "Water") {
                        $("#idWater").text(data[len].DisplayName);
                    } else {
                        if (data[len].WageType === "Air") {
                            $("#idAir").text(data[len].DisplayName);
                        } else {
                            if (data[len].WageType === "Gas") {
                                $("#idGas").text(data[len].DisplayName);
                            } else {
                                if (data[len].WageType === "Electricity") {
                                    $("#idElectricity").text(data[len].DisplayName);
                                } else {
                                    if (data[len].WageType === "Steam") {
                                        $("#idSteam").text(data[len].DisplayName);
                                    }
                                }
                            }
                        }
...
var generateGrid = function (id) {
    ...
    $("#" + costTable).bootstrapTable({
        ...
        columns: [
                    ...
                    { field: 'UOM', title: 'Currency', sortable: true, align: 'left' },
